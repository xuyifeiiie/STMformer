...
def softmax_kernel_transformation(data, is_query, projection_matrix=None, numerical_stabilizer=0.000001):
    data_normalizer = 1.0 / torch.sqrt(torch.sqrt(torch.tensor(data.shape[-1], dtype=torch.float32)))  # d_keys
    ...
    diag_data = torch.sum(diag_data, dim=len(data.shape) - 1)  # bnh
    diag_data = diag_data / 2.0
    diag_data = torch.unsqueeze(diag_data, dim=len(data.shape) - 1)  # bnh1

    last_dims_t = len(data_dash.shape) - 1  # m
    attention_dims_t = len(data_dash.shape) - 3  # n
    if is_query:
        data_dash = ratio * (
...
def add_conv_relational_bias(x, edge_index, b, trans='sigmoid'):
    ...
    for i in range(x.shape[2]):  # H
        ...
        value = torch.ones_like(row) * b_i * d_norm_in * d_norm_out  # E
...
class NodeFormerConv(nn.Module):
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.use_edge_loss:  # compute edge regularization loss on input adjacency
            row, col = adjs[0]
            d_in = degree(col, query.shape[1]).float()  # N
            d_norm = 1. / d_in[col]  # E
