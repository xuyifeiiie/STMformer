...
class STGraphEncoder(nn.Module):
    def __init__(
        ...
        if use_informer:
            from model.STSGT.attn import STSProbSparseSelfAttention
            ...
            from model.STSGT.attn import STSFullSelfAttention
...
class gcn_operation(nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        x = torch.einsum('bnm, mbc->nbc', adj.to(x.device), x)  # 12*N, B, Cin
...
class STSGCM(nn.Module):
    def __init__(self, in_dim, out_dims, num_of_vertices, d_model, n_heads, factor, attention_dropout,
    ...
    def forward(self, x: torch.Tensor, adj, mask: Optional[torch.Tensor] = None):
        """
        x: (12*N, B, Cin)
        mask: (B 12*N, 12*N)
        return: (12*N, B, Cout)
        ...
        if self.use_transformer:
            for i in range(len(self.out_dims)):
                transformer_temp = self.ST_GraphEncoders[i](input_transformer, mask)  # [12*N, B, C] torch.Size([3584, 2, 256])
                transformer_temp = self.gcn_operations[i](transformer_temp, adj, mask)  # [12*N, B, C]
                # print(transformer_temp.shape)
...
class STSGCL(nn.Module):
    def __init__(self, history, num_of_vertices, in_dim, out_dims, d_model, n_heads, factor, attention_dropout,
        ...
        self.STSGCMS = STSGCM(in_dim=self.in_dim,
    ...
    def forward(self, x, adj, mask=None):
        ...
        t = self.STSGCMS(t.permute(1, 0, 2), adj, mask)  # (12*N, B, Cin) -> (12*N, B, Cout)
...
class output_layer(nn.Module):
    def __init__(self, num_of_vertices, history, in_dim, num_features,
        ...
        self.predict_linear = nn.Linear(1, self.num_features)
    ...
    def forward(self, x):
        ...
        out2 = self.output_conv_2(out1)  # (B, hidden, N, 1) -> (B, horizon, N, 1)

        # print("Output Layer Shape after second conv2d: ", out2.shape)
        out2 = self.predict_linear(out2)  # (B, horizon, N, 1) -> (B, horizon, N, Cout)
        out = out2

        del out1, out2
        # print("Output Layer output shape: ", out.shape)

        return out  # B, horizon, N Cout
...
class STSGT(nn.Module):
    def __init__(self, settings, device):
        ...
        self.history_step = settings.history_steps
        self.num_features = settings.num_features
        self.num_of_vertices = settings.num_instances
        self.hidden_dims = settings.hidden_dims
        self.out_layer_dim = int(settings.d_model / 8)

        # The following is needed for the ST Self-Attentions
        self.d_model = int(settings.d_model / 8)
        self.n_heads = settings.n_heads
        self.factor = settings.factor
        self.attention_dropout = settings.dropout
        self.output_attention = settings.if_output_attention
        self.dropout = settings.dropout
        self.forward_expansion = int(settings.d_model / 8)

        # self.use_transformer = settings.use_transformer
        self.use_transformer = True
        # self.use_informer = settings.use_informer
        self.use_informer = True
        ...
        self.input_conv_1 = nn.Conv2d(in_channels=settings.num_features,
        ...
        self.predictLayer = output_layer(num_of_vertices=self.num_of_vertices,
                                         ...
                                         hidden_dim=settings.out_layer_dim,
                                         horizon=settings.predict_steps
                                         )
    ...
    def forward(self, x_enc, x_dec, x_mark_enc, x_mark_dec, adj_in, adj_out, instances_index_vm, mask=None):
        ...
        self.adj = construct_adj_optimized(self.adj)
        if self.use_mask:
            mask = torch.zeros_like(self.adj)
            mask[self.adj != 0] = self.adj[self.adj != 0]
            self.mask = nn.Parameter(mask, requires_grad=True)
        ...
        out = self.predictLayer(x)  # (B, T, N, Cout)
