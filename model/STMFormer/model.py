...
class STMFormer(nn.Module):
    ...
    def forward(self, x, y, x_mark_enc, x_mark_dec, adj_in, adj_out, instances_index_vm, mask=None):
        """
        ...
        #     # y = self.patch_embedding(y, if_keepdim=True)  # B num_patch N C*patch_len  2, 64, 56, 1536
        #     x = self.patch_features_embedding(x)  # B num_patch N C*patch_len -> B num_patch N embed_dim
        #     # y = self.patch_features_embedding(y)  # B num_patch N C*patch_len -> B num_patch N embed_dim
        #
        ...
        en_output, en_vm_attention_outer_list = self.encoder(x_res, adj_in, instances_index_vm)
        module_end_time = time.time()
        print("encoder compute time is {}".format(module_end_time - module_start_time))

        en_output = self.activation_layer(en_output)

        if self.distil:
            # B T N out_dims[-1] -> B T N d_model
            en_output = self.distil_linear(en_output)

        en_output = en_output + x_trend  # 之前没有这一步, 该步骤导致最优结果

        module_start_time = time.time()
        # B T N d_model, B T N de_in_dim(d_model) -> B T N d_model
        de_output, _ = self.decoder(en_output, x_res, en_vm_attention_outer_list, adj_in, instances_index_vm)
        module_end_time = time.time()
        print("decoder compute time is {}".format(module_end_time - module_start_time))

        de_output = self.activation_layer(de_output)

        de_output = de_output + x_trend
