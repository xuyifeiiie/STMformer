...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, adj, instances_index_vm, attn_mask=None):
        ...
        trend_fin = torch.zeros_like(x).to(x.device)
        ...
        if self.decomp:
            x_res, x_trend_part = self.series_decomp(x_res)
        elif self.multidecomp:
            x_res, x_trend_part = self.series_decomp(x_res)
        else:
            x_res = x_res
            x_trend_part = torch.zeros_like(x)
        # trend_fin += self.trend_layer0(x_trend_part)
        x_raw = x_res
        ...
        if self.decomp:
            ...
            x_res = x_res
            x_trend_part = torch.zeros_like(x)
        # trend_fin += self.trend_layer1(x_trend_part)
        x_raw = x_res
        ...
        if self.decomp:
            ...
            x_res = x_res
            x_trend_part = torch.zeros_like(x)
        # trend_fin += self.trend_layer2(x_trend_part)
        # x_res = x_res + trend_fin  # latest,
        x_raw = x_res
        x_res = self.feedforward(x_res)

        if not self.distil:
            x_res = self.norm_layer(x_raw, x_res)  # B T N d_model
            x = x_res
        else:
            x = x_res
            x = self.layernorm(x)  # B T N after_d_model
