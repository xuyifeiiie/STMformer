...
from layer.temporal import TemporalRelationModule, TemporalMaskedSelfAttentionLayer
from utils.masking import TimeSeriesMask


class DecoderLayer(nn.Module):
    def __init__(self, num_features, embed_dim, en_in_dim, de_in_dim, out_dim, hidden_dim, d_model=768,
                 after_d_model=128, n_heads=8, num_of_instances=42, num_attention_layers=2, num_en_layers=3,
                 num_de_layers=3, factor=6, kernels=12, dropout=0.1, attention='prob', embed='fixed', freq='h',
        ...
        self.masked_temporal_self_attention = TemporalMaskedSelfAttentionLayer(attention=self.attention,
                                                                               ...
                                                                               num_instances=self.num_of_instances)
    ...
    def forward(self, x, y, en_vm_attention_outer_list, adj, instances_index_vm, attn_mask=None):
        ...
        """
        B, T, N, D = x.shape
        # temporal_mask = TimeSeriesMask(B, self.n_heads, T, x.device)
        x1, _ = self.masked_temporal_self_attention(y, y, y, attn_mask)

        x1 = self.norm_layer(y, x1)
        x1_raw = x1

        x1, _ = self.self_attention(x, x1, x1, attn_mask)
        ...
        for vm_attention_layer in self.vm_attention_modules:
            ...
            x1, vm_attention = vm_attention_layer(x1, instances_index_vm, attn_mask)
        ...
        for spatial_message_layer in self.spatial_message_modules:
            x1 = spatial_message_layer(x1, adj)
...
class Decoder(nn.Module):
    def __init__(self, num_features, embed_dim, en_in_dim, de_in_dim, out_dim, hidden_dim, d_model=512,
                 after_d_model=128, n_heads=8, num_of_instances=56, num_attention_layers=2, num_en_layers=3,
                 num_de_layers=3, factor=6, kernels=12, dropout=0.1, attention='prob', embed='fixed', freq='h',
                 activation='gelu', use_mask=True, if_output_attention=True, co_train=False, distil=True, mix=True,
                 history_steps=16, predict_steps=16, strides=16, device=torch.device('cuda:0')):
        ...
        self.decoder_layer_stack = nn.ModuleList([DecoderLayer(num_features=self.num_features,
                                                  ...
                                                  for _ in range(self.num_de_layers)])

        self.dropout_layer = nn.Dropout(self.dropout)

    def forward(self, x, y, en_vm_attention_outer_list, adj, instances_index_vm):
        """
        :param x: B T N d_model
        ...
        for decoder_layer in self.decoder_layer_stack:
            x, vm_attention_list = decoder_layer(x, y, en_vm_attention_outer_list, adj, instances_index_vm)
