...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, y, en_vm_attention_outer_list, adj, instances_index_vm, attn_mask=None):
        ...
        if self.decomp:
            ...
            y_res = y
            y_trend = torch.zeros_like(y_res)

        y_trend = self.trend_layer0(y_trend)
        y_raw = y_res
        ...
        if self.decomp:
            y_res, y_trend1 = self.series_decomp(y_res)
        elif self.multidecomp:
            y_res, y_trend1 = self.series_decomp(y_res)
        else:
            y_res = y_res
            y_trend1 = torch.zeros_like(y_res)

        y_trend1 = self.trend_layer1(y_trend1)
        y_trend = y_trend + y_trend1
        y_raw = y_res
        ...
        y_trend1 = self.trend_layer2(y_trend1)
        y_trend = y_trend + y_trend1
        y_raw = y_res
        ...
        y_trend1 = self.trend_layer3(y_trend1)
        y_trend = y_trend + y_trend1
        y_raw = y_res
        ...
        # print("spatial_message_modules of decoder compute time is {}".format(module_end_time - module_start_time))
        ...
        y_trend1 = self.trend_layer4(y_trend1)
        y_trend = y_trend + y_trend1
        y_raw = y_res
        ...
        # y_res = self.alpha(self_res) + self.alpha(cross_res)
        ...
        y_trend1 = self.trend_layer5(y_trend1)
        y_trend = y_trend + y_trend1
        y_raw = y_res
