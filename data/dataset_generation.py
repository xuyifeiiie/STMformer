...
def generate_train_val_test_for_metric(args, task_dataset_path):
    ...
    for category in tqdm.tqdm(category_list):
        ...
        if sys.platform.startswith('win'):
            ...
            category_metric_path = category_path + category + "_all_nodes_metric_labeled_df" + ".csv"
            category_instance_path = category_path + category + "_instances_index_vm_df" + ".csv"
            category_connection_path = category_path + category + "_all_connection_df" + ".csv"
        elif sys.platform.startswith('linux'):
            cat_short = category.split('/')[-1]
            category_path = category + "/"
            category_metric_path = category_path + cat_short + "_all_nodes_metric_labeled_df" + ".csv"
            category_instance_path = category_path + cat_short + "_instances_index_vm_df" + ".csv"
            category_connection_path = category_path + cat_short + "_all_connection_df" + ".csv"
        ...
        category_connection_df = pd.read_csv(category_connection_path, sep=',', header=0, index_col=0)

        category_metric_df = preprocess_df(category_metric_df)

        x, y, adj_in, adj_out, l1, l2, l3, instances_count, arr_min, arr_max \
            = generate_graph_seq2seq_io_data(category_metric_df,
        ...
        if min_final is None:
            ...
            min_final, max_final = update_min_max(min_final, max_final, arr_min, arr_max)

        if args.if_shuffle:
            x, y, adj_in, adj_out, l1, l2, l3, instances_count = shuffle_before_load(x, y, adj_in, adj_out, l1, l2, l3,
                                                                                     instances_count)
        ...
        for cat in ["train", "val", "test"]:
            ...
            if sys.platform.startswith('win'):
                npz_path = './' + os.path.join(category, "{}_{}_{}.npz".format(cat_short, cat, sample_rate_str))
            elif sys.platform.startswith('linux'):
                npz_path = os.path.join(category, "{}_{}_{}.npz".format(cat_short, cat, sample_rate_str))
            ...
            print("{}_{} dataset has been processed and saved successfully!".format(cat_short, cat))

    # free mem
    x, y, adj_in, adj_out, l1, l2, l3, instances_count = None, None, None, None, None, None, None, None
    x_train, x_val, x_test = None, None, None
    y_train, y_val, y_test = None, None, None
    adj_in_train, adj_in_val, adj_in_test = None, None, None
    adj_out_train, adj_out_val, adj_out_test = None, None, None
    l1_train, l1_val, l1_test = None, None, None
    l2_train, l2_val, l2_test = None, None, None
    l3_train, l3_val, l3_test = None, None, None
    instances_count_train, instances_count_val, instances_count_test = None, None, None
    _x, _y = None, None
    _adj_in, _adj_out = None, None
    _l1, _l2, _l3 = None, None, None
    _instances_count = None
    ...
    #                                                      "{}_{}_{}.npz".format(cat_short, 'train', sample_rate_str))
    #         elif sys.platform.startswith('linux'):
    #             cat_short = category.split('/')[-1]
    #             npz_path = args.data_path + os.path.join(category,
    #                                                      "{}_{}_{}.npz".format(cat_short, 'train', sample_rate_str))
    ...
    #         x_train_list.append(data_x)
    #
    #     x_train_all = np.concatenate(x_train_list, axis=0)
    #     x_mean, x_std = compute_reshape_mean_std_for_4_dimension_data(x_train_all)
    ...
    #                 y=data_y,
    #                 adj_in=data_adj_in,
    #                 adj_out=data_adj_out,
    #                 l1=data_l1,
    #                 l2=data_l2,
    #                 l3=data_l3,
    #                 instances_count=data_instances_count
    #             )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_name", type=str, default="short_term_forecast",
                        help="Task name", )
    parser.add_argument("--seq_length_x", type=int, default=16, help="Input Sequence Length.", )
    parser.add_argument("--seq_length_y", type=int, default=16, help="Output Sequence Length.", )
